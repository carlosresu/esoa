...
def install_requirements(req_path: str):
    """Install requirements with the same Python interpreter running this script, suppressing pip output."""
    if not req_path:
        return
...
def run_r_scripts():
    ...
    for script in scripts:
        subprocess.run([rscript, script], check=True, cwd=atcd_dir, stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL)
...
# - change 1: edit <file_path> by downloading the below
#   <file download link here>
# - change 2: edit <file_path> by downloading the below
#   <file download link here>
# - <and so on...>
...
def main_entry():
    ...
    parser.add_argument("--pnf", default="inputs/pnf.csv", help="Path to raw PNF CSV (default: inputs/pnf.csv)")
    parser.add_argument("--esoa", default="inputs/esoa.csv", help="Path to raw eSOA CSV (default: inputs/esoa.csv)")
    parser.add_argument("--out", default="esoa_matched.csv", help="Final matched CSV filename (always written to ./outputs/)")
    parser.add_argument("--requirements", default="requirements.txt", help="Path to requirements file (default: requirements.txt)")
