...
File outputs:
  • pipelines/drugs/scripts/match_outputs_drugs.py writes ./outputs/drugs/summary.txt (overwritten each run).
  • Finally runs pipelines/drugs/scripts/resolve_unknowns_drugs.py to analyze unmatched terms and write its outputs under ./outputs/drugs.
"""
...
PIPELINE_INPUTS_SUBDIR = Path(DEFAULT_INPUTS_DIR) / "drugs"
PIPELINE_OUTPUTS_SUBDIR = Path(OUTPUTS_DIR) / "drugs"
...
def main_entry() -> None:
    ...
    parser.add_argument("--esoa", default=None, help="Path to eSOA CSV (defaults to concatenated inputs/drugs/esoa_pt_*.csv)")
