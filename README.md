...
- Loads the curated DrugBank generics export generated by `dependencies/drugbank/drugbank.R` (copied to `inputs/drugs_and_medicine/generics.csv` via `python -m pipelines.drugs.scripts.run_drugbank_drugs`), normalizes to lowercase ASCII, and indexes by token.
...
- Optionally loads `inputs/drugs_and_medicine/fda_food_products.csv` (scraped via `pipelines/drugs/scripts/fda_ph_food_scraper_drugs.py`) and builds an automaton of brand and product strings.
