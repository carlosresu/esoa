...
- Loads the curated DrugBank generics export generated by `dependencies/drugbank_generics/drugbank_all.R`, which writes `dependencies/drugbank_generics/output/drugbank_generics.csv` and mirrors the CSVs into `inputs/drugs/` so downstream stages can normalize to lowercase ASCII and index by token.
