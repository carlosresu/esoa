#!/usr/bin/env python
# -*- coding: utf-8 -*-
"""Match Annex F entries to ATC codes using PNF, DrugBank, and WHO lexicons (Polars-first)."""
...
from ..constants import PIPELINE_INPUTS_DIR, PIPELINE_OUTPUTS_DIR

REFERENCE_ROWS_SHARED: list[dict] | None = None
REFERENCE_INDEX_SHARED: dict[str, list[int]] | None = None
BRAND_PATTERNS_SHARED: list[tuple[re.Pattern, str]] | None = None
GENERIC_PHRASES_SHARED: list[str] | None = None
GENERIC_AUTOMATON_SHARED = None
MIXTURE_LOOKUP_SHARED: dict[str, list[dict]] | None = None
DRUGBANK_BY_ID_SHARED: dict[str, list[dict]] | None = None

DEFAULT_ANNEX_LEX_PATH = PIPELINE_INPUTS_DIR / "annex_f_lexicon.csv"
DEFAULT_ANNEX_RAW_PATH = PIPELINE_INPUTS_DIR / "annex_f.csv"
DEFAULT_DRUGBANK_PATH = PIPELINE_INPUTS_DIR / "drugbank_generics_master.csv"
DEFAULT_PNF_PATH = PIPELINE_INPUTS_DIR / "pnf_lexicon.csv"
DEFAULT_WHO_PATH = PIPELINE_INPUTS_DIR / "who_atc_2025-11-20.csv"
DEFAULT_FDA_BRAND_PATH = PIPELINE_INPUTS_DIR / "fda_drug_2025-11-12.csv"
DEFAULT_MIXTURE_PATH = PIPELINE_INPUTS_DIR / "drugbank_mixtures_master.csv"
...
def _init_shared(
    ...
    global REFERENCE_ROWS_SHARED
    global REFERENCE_INDEX_SHARED
    global BRAND_PATTERNS_SHARED
    global GENERIC_PHRASES_SHARED
    global GENERIC_AUTOMATON_SHARED
    global MIXTURE_LOOKUP_SHARED
    global DRUGBANK_BY_ID_SHARED
    REFERENCE_ROWS_SHARED = reference_rows
    REFERENCE_INDEX_SHARED = reference_index
    BRAND_PATTERNS_SHARED = brand_patterns
    GENERIC_PHRASES_SHARED = generic_phrases
    GENERIC_AUTOMATON_SHARED = _build_aho_automaton(generic_phrases) if generic_phrases else None
    MIXTURE_LOOKUP_SHARED = mixture_lookup
    DRUGBANK_BY_ID_SHARED = drugbank_refs_by_id


def _process_annex_row_worker(annex_row: dict) -> tuple[dict, list[dict], list[dict]]:
    reference_rows = REFERENCE_ROWS_SHARED or []
    reference_index = REFERENCE_INDEX_SHARED or {}
    brand_patterns = BRAND_PATTERNS_SHARED or []
    generic_automaton = GENERIC_AUTOMATON_SHARED
    mixture_lookup = MIXTURE_LOOKUP_SHARED or {}
    drugbank_refs_by_id = DRUGBANK_BY_ID_SHARED or {}
...
def run_annex_atc_match(
    *,
    annex_lex_path: Path = DEFAULT_ANNEX_LEX_PATH,
    annex_raw_path: Path = DEFAULT_ANNEX_RAW_PATH,
    drugbank_path: Path = DEFAULT_DRUGBANK_PATH,
    pnf_path: Path = DEFAULT_PNF_PATH,
    who_path: Path = DEFAULT_WHO_PATH,
    fda_brand_path: Path = DEFAULT_FDA_BRAND_PATH,
    mixture_path: Path = DEFAULT_MIXTURE_PATH,
    outputs_dir: Path = PIPELINE_OUTPUTS_DIR / "drugs",
    max_workers: int | None = None,
    ...
    thread_flag = bool(int(os.environ.get("ESOA_MATCH_ANNEX_THREADS", "0"))) if use_threads is None else bool(use_threads)
    ...
    match_path = outputs_dir / "annex_f_with_atc.csv"
    ties_path = outputs_dir / "annex_f_atc_ties.csv"
    unresolved_path = outputs_dir / "annex_f_atc_unresolved.csv"

    _write_csv_and_parquet(_reorder_columns(match_df), match_path)
    _write_csv_and_parquet(_reorder_columns(tie_df), ties_path)
    _write_csv_and_parquet(_reorder_columns(unresolved_df), unresolved_path)
    ...
    return match_df, tie_df, unresolved_df
