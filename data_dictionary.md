...
`run_drugs_and_medicine.py` now orchestrates dependency bootstrapping, input preparation, and the post-run execution of `pipelines/drugs/scripts/resolve_unknowns_drugs.py`, so the companion outputs mentioned below (e.g., `outputs/drugs_and_medicine/unknown_words.csv`) are regenerated automatically each time the pipeline completes.
