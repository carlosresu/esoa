...
OUTPUT_COLUMNS = [
    "raw_text","normalized","molecules_recognized","molecules_recognized_count","probable_brands","dose_recognized","dose_kind_detected","route","form","present_in_pnf","present_in_who","probable_atc","generic_id","molecule_token","pnf_hits_count","pnf_hits_tokens","who_molecules","who_atc_codes","who_atc_count","route_evidence","dosage_parsed","selected_form","selected_variant","dose_sim","looks_combo_raw","looks_combo_final","combo_reason","combo_segments_total","combo_id_molecules","combo_unknown_in_pnf","combo_unknown_in_who","combo_unknown_in_both","atc_code_final","confidence","bucket_final","why_final",
]

def write_outputs(out_df: pd.DataFrame, out_csv: str) -> str:
    out_small = out_df[OUTPUT_COLUMNS].copy()
    ...
    if not nr.empty:
        ...
        def fam_order(s):
            if s.startswith("ValidMoleculeWithATC/NotInPNF:"): return (0, s)
            if s.startswith("Candidate:"): return (1, s)
            if s == "BrandOnly/NoGenericInPNF/NoGenericInWHO": return (2, s)
            return (3, s)
